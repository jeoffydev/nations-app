    ...
    console.log("instrumentsArray", instrumentsArray);
        ...
        const instrumentList = instrumentsArray.map( (ins) =>    
            <li className="list-group-item" key={ins.id}>  
                 {insSel.indexOf(ins.id) !== -1   ?  <InputCheckBox  name='instrumentCheckBox' id={'check' + ins.id} type='checkbox'   label={ins.instrumentName} value={ins.id} check={false}  onChange={onChange}     />   :   <InputCheckBox  name='instrumentCheckBox' id={'check' + ins.id} type='checkbox'   label={ins.instrumentName} value={ins.id} check={true}  onChange={onChange}     />  }   
            </li>
        ) 
